...
class SeedOfficeWorkLogs1405FarOrdib extends Command
{
    protected $signature = 'attendance:seed-office-1405-far-ordib
        {--project-id=489 : شناسه پروژه پیش‌فرض در صورت خالی بودن پروژه پرسنل}
        {--dry-run : فقط گزارش تعداد روزها، بدون ذخیره}';

    protected $description = 'ثبت کارکرد روزانه اداری مهدی عسگری و میلاد رحیمی برای فروردین تا اردیبهشت 1405 بر اساس تقویم و شیفت اداری';

    private const IMPORT_BATCH = 'WORKLOG-SEED-1405-FAR-ORDIB';

    private const ATTENDANCE_SOURCE = 'worklog-seed-1405-far-ordib';

    private const EMPLOYEE_CODES = ['EMP-001', 'EMP-002', 'P-1404-MA', 'P-1404-MR'];
    ...
    public function handle(): int
    {
        ...
        $end = Carbon::parse(jalaliToGregorianDateSafe(1405, 2, 31))->endOfDay();
        ...
        $group = WorkGroup::updateOrCreate(
            ...
            [
                ...
                'is_active' => true,
                'description' => 'گروه اداری برای کارکرد فروردین تا اردیبهشت 1405',
            ]
        );
        ...
        $this->info('ثبت کارکرد فروردین تا اردیبهشت 1405 انجام شد.');
    ...
    private function resolveEmployees()
    {
        $byCode = Employee::query()
            ->whereIn('personnel_code', ['EMP-001', 'EMP-002'])
            ->orderBy('id')
            ->get();
