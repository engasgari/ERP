<div class="py-4 py-md-5"><div class="bg-white rounded-lg shadow-md p-3 p-md-4 space-y-4">
    ...
    <div class="table-responsive"><table class="erp-ui-data-table w-full"><thead><tr><th>پرسنل</th><th>نوع</th><th>عنوان</th><th>صدور</th><th>انقضا</th><th>فایل</th><th>عملیات</th></tr></thead><tbody>@forelse($documents as $document)<tr wire:key="document-{{ $document->id }}"><td>{{ $document->employee->full_name }}</td><td>{{ $document->document_type }}</td><td>{{ $document->title }}</td><td>{{ formatJalaliDateSafe($document->issued_at, '-') }}</td><td>{{ formatJalaliDateSafe($document->expires_at, '-') }}</td><td dir="ltr">{{ $document->file_path ?: '-' }}</td><td><x-erp.ui.row-actions><x-erp.ui.row-action icon="edit" label="ویرایش" wire:click="edit({{ $document->id }})" /><x-erp.ui.row-action icon="delete" label="حذف" tone="danger" wire:click="delete({{ $document->id }})" wire:confirm="حذف شود؟" /></x-erp.ui.row-actions></td></tr>@empty<tr><td colspan="7" class="text-center py-6 text-slate-500">مدرکی ثبت نشده است.</td></tr>@endforelse</tbody></table></div>
    {{ $documents->links() }}
</div></div>
