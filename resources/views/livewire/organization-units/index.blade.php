<div class="py-4 py-md-5">
    <div class="bg-white rounded-lg shadow-md p-3 p-md-4 space-y-4">
        ...
        <div class="table-responsive"><table class="erp-ui-data-table w-full"><thead><tr><th>کد</th><th>عنوان</th><th>نوع</th><th>والد</th><th>مرکز هزینه</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>@forelse($units as $unit)<tr wire:key="unit-{{ $unit->id }}"><td>{{ $unit->code }}</td><td>{{ $unit->title }}</td><td>{{ $unit->type }}</td><td>{{ $unit->parent?->title ?: '-' }}</td><td>{{ $unit->cost_center_code ?: '-' }}</td><td>{{ $unit->is_active ? 'فعال' : 'غیرفعال' }}</td><td><x-erp.ui.row-actions><x-erp.ui.row-action icon="edit" label="ویرایش" wire:click="edit({{ $unit->id }})" /><x-erp.ui.row-action icon="delete" label="حذف" tone="danger" wire:click="delete({{ $unit->id }})" wire:confirm="حذف شود؟" /></x-erp.ui.row-actions></td></tr>@empty<tr><td colspan="7" class="text-center py-6 text-slate-500">رکوردی ثبت نشده است.</td></tr>@endforelse</tbody></table></div>
        {{ $units->links() }}
    </div>
</div>
