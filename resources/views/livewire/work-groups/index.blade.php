<div class="bg-white rounded-lg shadow-md p-3 p-md-4 space-y-4">
    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center justify-content-between gap-3">
        <h3 class="text-lg font-bold mb-0">لیست گروه‌های کاری</h3>
        <a href="{{ route('work-groups.create') }}" class="erp-action-btn erp-action-edit text-center">ثبت گروه کاری</a>
    </div>
    ...
    <form wire:submit.prevent class="erp-ui-filter-bar">
        <div class="row g-2 g-md-3 align-items-end">
            <label class="erp-filter-field col-12 col-md-6 col-lg-7">جستجو
                <input wire:model.live.debounce.400ms="search" placeholder="کد، نام، شیفت یا تقویم">
            </label>
        ...
        <table class="erp-ui-data-table min-w-full">
            ...
            @forelse($groups as $group)
                <tr wire:key="group-{{ $group->id }}">
                    ...
                    <td>
                        <input class="form-control form-control-sm" value="{{ $group->name }}" wire:change="updateField({{ $group->id }}, 'name', $event.target.value)">
                    </td>
                    <td>{{ $group->shift?->name ?: '-' }}</td>
                    <td>{{ $group->calendar?->name ?: '-' }}</td>
                    <td>
                        <select class="form-select form-select-sm" wire:change="updateField({{ $group->id }}, 'is_active', $event.target.value)">
                            <option value="1" @selected($group->is_active)>فعال</option>
                            <option value="0" @selected(! $group->is_active)>غیرفعال</option>
                        </select>
                    </td>
                    <td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center text-slate-500 py-6">رکوردی ثبت نشده است.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
