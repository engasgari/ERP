<div dir="rtl" class="space-y-6">
    ...
    <div class="rounded-lg bg-white p-4 shadow-md sm:p-6">
        <div class="mb-4">
            <h3 class="text-lg font-bold text-slate-900">ثبت درخواست ماموریت</h3>
            <p class="mt-1 text-sm text-slate-500">ماموریت تایید شده در خلاصه کارکرد به عنوان کارکرد مجاز دیده می‌شود.</p>
        </div>
        ...
        <form wire:submit="save" class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label class="block">
                <span class="text-sm font-medium text-slate-700">مقصد</span>
                <input wire:model="destination" class="mt-1 w-full rounded-md border-gray-300 text-right">
            </label>
            ...
            @if($request_type === 'hourly')
                ...
                <label class="block">
                    ...
                    <input type="number" step="0.25" wire:model="hours" class="mt-1 w-full rounded-md border-gray-300 text-right">
                </label>
            @else
                <label class="block">
                    <span class="text-sm font-medium text-slate-700">تعداد روز</span>
                    <input type="number" step="0.5" wire:model="total_days" class="mt-1 w-full rounded-md border-gray-300 text-right">
                </label>
            @endif

            <label class="block">
                <span class="text-sm font-medium text-slate-700">پروژه</span>
                <select wire:model="project_id" class="mt-1 w-full rounded-md border-gray-300 text-right">
            ...
            <label class="block">
                <span class="text-sm font-medium text-slate-700">مرکز هزینه</span>
                <select wire:model="cost_center_id" class="mt-1 w-full rounded-md border-gray-300 text-right">
            ...
            <label class="block md:col-span-2 xl:col-span-3">
                <span class="text-sm font-medium text-slate-700">شرح</span>
                <input wire:model="description" class="mt-1 w-full rounded-md border-gray-300 text-right">
            </label>
    ...
    <div class="rounded-lg bg-white p-4 shadow-md sm:p-6">
        <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <h3 class="text-lg font-bold text-slate-900">درخواست‌های ماموریت</h3>
                <p class="mt-1 text-sm text-slate-500">ماموریت‌های تایید شده در موتور حضور و غیاب لحاظ می‌شوند.</p>
            </div>
            <form wire:submit.prevent class="grid grid-cols-1 gap-2 sm:grid-cols-4 lg:min-w-[680px]">
                <input wire:model.live.debounce.400ms="search" placeholder="جستجوی پرسنل یا مقصد" class="rounded-md border-gray-300 text-right sm:col-span-2">
        ...
        <div class="overflow-x-auto">
            <table class="erp-ui-data-table min-w-full">
                ...
                @forelse($missions as $mission)
                    <tr wire:key="mission-{{ $mission->id }}">
                        <td>{{ $mission->employee?->full_name }}</td>
