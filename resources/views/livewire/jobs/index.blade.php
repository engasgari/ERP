<div class="py-4 py-md-5"><div class="bg-white rounded-lg shadow-md p-3 p-md-4 space-y-4">
    ...
    <div class="table-responsive"><table class="erp-ui-data-table w-full"><thead><tr><th>کد</th><th>عنوان</th><th>شرح</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>@forelse($jobs as $job)<tr wire:key="job-{{ $job->id }}"><td>{{ $job->code }}</td><td>{{ $job->title }}</td><td>{{ $job->description ?: '-' }}</td><td>{{ $job->is_active ? 'فعال' : 'غیرفعال' }}</td><td><x-erp.ui.row-actions><x-erp.ui.row-action icon="edit" label="ویرایش" wire:click="edit({{ $job->id }})" /><x-erp.ui.row-action icon="delete" label="حذف" tone="danger" wire:click="delete({{ $job->id }})" wire:confirm="حذف شود؟" /></x-erp.ui.row-actions></td></tr>@empty<tr><td colspan="5" class="text-center py-6 text-slate-500">رکوردی ثبت نشده است.</td></tr>@endforelse</tbody></table></div>
    {{ $jobs->links() }}
</div></div>
