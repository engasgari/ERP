    @php
        ...
        $routes = $routes ?? [];
        $urlPrint = $routes['print'] ?? route('invoices.print', $invoice);
        $urlPdf = $routes['pdf'] ?? route('invoices.pdf', $invoice);
        $urlExcel = $routes['excel'] ?? route('invoices.excel', $invoice);
    ...
    @endphp
        ...
        <div class="invoice-print-actions">
            @if($embedded)
                <button type="button" id="invoice-show-embedded-close">بستن</button>
            @else
                <a href="{{ route('invoices.index') }}">بازگشت به فهرست</a>
            @endif
            @if($embedded && (! ($crmContext ?? false) || ($invoice->document_type === 'proforma' && $invoice->status === 'draft')))
                <button type="button" id="invoice-show-embedded-edit">
                    {{ $invoice->document_type === 'proforma' ? 'ویرایش پیش‌فاکتور' : 'ویرایش فاکتور' }}
                </button>
            @elseif(! $embedded)
                <a href="{{ route('invoices.edit', $invoice) }}">ویرایش فاکتور</a>
            @endif
            @unless($crmContext ?? false)
                <a href="{{ $urlPrint }}" @if($embedded) data-embedded-download @else target="_blank" data-no-spa @endif>چاپ نهایی</a>
                <a href="{{ $urlPdf }}" @if($embedded) data-embedded-download @else target="_blank" data-no-spa @endif>PDF</a>
            @endunless
            <a href="{{ $urlExcel }}" @if($embedded) data-embedded-download @else target="_blank" data-no-spa @endif>Excel</a>
            @unless($crmContext)
            <form method="POST" action="{{ route('invoices.destroy', $invoice) }}" class="invoice-embedded-action-form" @unless($embedded) onsubmit="return confirm('فاکتور و همه سندهای مالی و انبار وابسته حذف شوند؟')" @endunless>
            @csrf
            @method('DELETE')
        ...
        @elseif($invoice->status === 'confirmed' && ! $invoice->settled_at)
            <form method="POST" action="{{ route('invoices.settle', $invoice) }}" class="invoice-embedded-action-form">
                @csrf
                <button type="submit" class="primary">تسویه فاکتور</button>
            </form>
        @elseif($invoice->settled_at)
            <form method="POST" action="{{ route('invoices.unsettle', $invoice) }}" class="invoice-embedded-action-form">
                @csrf
    ...
    @if($embedded)
        <div id="invoice-embedded-action-error" class="invoice-inventory-error" hidden></div>
    @endif
        ...
        <table class="tax-lines-table">
            ...
            @foreach($invoice->lines as $line)
                ...
                <tr>
                    ...
                    <td>{{ formatMoney((float) $line->tax_rate, 2) }}٪</td>
            ...
            <tr>
                ...
                <th>{{ formatMoney($rowBaseTotal) }}</th>
                <th>{{ formatMoney($discountTotal) }}</th>
                <th>{{ formatMoney(max($rowBaseTotal - $discountTotal, 0)) }}</th>
                <th></th>
                <th>{{ formatMoney($taxTotal) }}</th>
                <th>{{ formatMoney($finalTotal) }}</th>
            </tr>
            </tfoot>
        </table>
    ...
    <script>
        ...
        if (@json($embedded)) {
            ...
            const embeddedError = document.getElementById('invoice-embedded-action-error');

            document.getElementById('invoice-show-embedded-close')?.addEventListener('click', () => {
                window.parent.postMessage({ type: 'invoice-show-cancel' }, window.location.origin);
            });

            document.getElementById('invoice-show-embedded-edit')?.addEventListener('click', () => {
                window.parent.postMessage({
                    type: 'invoice-open-edit',
                    invoiceId,
                    documentType: @json($invoice->document_type),
                    crmContext: @json($crmContext ?? false),
                }, window.location.origin);
            });

            document.querySelectorAll('[data-embedded-download]').forEach((link) => {
                link.addEventListener('click', (event) => {
                    event.preventDefault();

                    const url = link.href;
                    const topDocument = window.top.document;
                    const anchor = topDocument.createElement('a');
                    anchor.href = url;
            ...
            document.querySelectorAll('.invoice-embedded-action-form').forEach((form) => {
                form.addEventListener('submit', async (event) => {
                    event.preventDefault();

                    const isDelete = form.querySelector('[name="_method"]')?.value === 'DELETE'
                        || form.getAttribute('method')?.toUpperCase() === 'DELETE';

                    if (isDelete && !window.confirm('فاکتور و همه سندهای مالی و انبار وابسته حذف شوند؟')) {
                        return;
                    }

                    if (embeddedError) {
                        embeddedError.hidden = true;
                        embeddedError.textContent = '';
                    }
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            const errors = payload.errors
                                ...
                                : [payload.message || 'عملیات انجام نشد.'];
                        ...
                        window.parent.postMessage({
                            ...
                            reloadShowId: payload.reload_show_id ?? null,
                        }, window.location.origin);
                    } catch (error) {
                        if (embeddedError) {
                            embeddedError.hidden = false;
                            embeddedError.textContent = error.message || 'عملیات انجام نشد.';
                        }
                    } finally {
                        if (submitButton) {
