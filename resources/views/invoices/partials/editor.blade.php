    @php
        ...
        $catalogItems = $items;
        if ($invoice?->lines) {
            $invoice->loadMissing('lines.item.unit');
            $catalogItems = $items
                ->concat($invoice->lines->pluck('item')->filter())
                ->unique('id')
                ->values();
        }
        $salePriceService = app(\App\Services\ItemSalePriceService::class);
        $lastSaleUnitPrices = $direction === 'sale'
            ? $salePriceService->saleUnitPricesForCatalog($catalogItems, $invoice?->id)
            : [];
        $itemData = $catalogItems->mapWithKeys(function ($item) use ($direction, $lastSaleUnitPrices, $salePriceService) {
            $price = $direction === 'purchase'
                ? (float) ($item->purchase_price ?? 0)
                : $salePriceService->resolveSaleUnitPrice($item, $lastSaleUnitPrices);
        ...
        $itemOptions = itemSearchSelectOptions($catalogItems);
        ...
        $invoiceDateValue = old('invoice_date')
            ...
            : ($invoice
                ? gregorianToJalaliDate($invoice->invoice_date)
                : (isset($draftDefaults['invoice_date'])
                    ? gregorianToJalaliDate($draftDefaults['invoice_date'])
                    : todayJalaliDate()));
        $formAction = $formAction ?? ($isEdit ? route('invoices.update', $invoice) : route('invoices.store'));
    @endphp

    <form method="POST" action="{{ $formAction }}" class="invoice-editor space-y-4">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        @if($embedded)
            <div id="invoice-embedded-errors" class="invoice-error" hidden></div>
        @endif
                    ...
                    <span class="party-select-row">
                        <select name="party_id" id="invoice-party-select" required>
                            ...
                            @foreach($parties as $party)
                                <option value="{{ $party->id }}" @selected(old('party_id', $draftDefaults['party_id'] ?? $invoice?->party_id) == $party->id)>{{ $party->name }}</option>
                            @endforeach
                        </select>
                ...
                <label>
                    ...
                    <select name="project_id">
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" @selected(old('project_id', $draftDefaults['project_id'] ?? $invoice?->project_id) == $project->id)>{{ $project->code }} - {{ $project->name }}</option>
                        @endforeach
                    </select>
                </label>
                <label>
        ...
        <section class="invoice-panel">
            ...
            <div class="invoice-actions">
                @if($embedded)
                    <button type="button" id="invoice-embedded-cancel">انصراف</button>
                @else
    ...
    <template id="invoice-line-template">
        <tr class="invoice-line">
            ...
            <td>
                <div class="erp-search-select" data-erp-search-select data-options-source="invoice-items">
            ...
            <td><input class="line-quantity" type="text" inputmode="decimal" autocomplete="off" dir="ltr" data-erp-money="0" data-erp-number="0"></td>
            <td><input class="line-price" type="text" inputmode="decimal" autocomplete="off" dir="ltr" data-erp-money="0" data-erp-number="0"></td>
            <td><input class="line-discount" type="text" inputmode="decimal" autocomplete="off" dir="ltr" value="0" data-erp-money="0" data-erp-number="0"></td>
            <td><input class="line-tax-rate" type="text" inputmode="decimal" autocomplete="off" dir="ltr" value="10" data-erp-money="0" data-erp-number="0"></td>
    ...
    (() => {
        ...
        window.__erpInvoiceItemOptions = @json($itemOptions);
        ...
        function isRowStarted(row) {
            return Boolean(getLineItemInput(row).value);
        }
        ...
        function recalculateInvoice() {
            ...
            linesBody.querySelectorAll('.invoice-line').forEach((row) => {
                const item = getLineItemInput(row);
        ...
        function isInvoiceLineNumberInput(input) {
            return input?.classList.contains('line-quantity')
                || input?.classList.contains('line-price')
                || input?.classList.contains('line-discount')
                || input?.classList.contains('line-tax-rate');
        }

        function normalizeInvoiceLineNumberInput(input) {
            if (!isInvoiceLineNumberInput(input) || input.readOnly) {
                return;
            }

            const raw = normalizeEditableNumber(input.value);
            ...
            if (typeof cursor === 'number' && input.setSelectionRange) {
                const nextCursor = Math.min(cursor, raw.length);
                input.setSelectionRange(nextCursor, nextCursor);
            }
        }

        linesBody.addEventListener('input', (event) => {
            normalizeInvoiceLineNumberInput(event.target);
            recalculateInvoice();
        });
        linesBody.addEventListener('focusin', (event) => {
            if (isInvoiceLineNumberInput(event.target)) {
        ...
        function bootstrapInvoiceLines() {
            if (oldLines.length) {
            ...
            linesBody.querySelectorAll('.line-quantity, .line-price, .line-discount, .line-tax-rate').forEach((input) => {
                input.dataset.erpMoney = '0';
                input.dataset.erpNumber = '0';
            });
        }

        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', bootstrapInvoiceLines);
        } else {
            bootstrapInvoiceLines();
        }

        if (@json($embedded)) {
            const editorForm = document.querySelector('.invoice-editor');
            const embeddedErrors = document.getElementById('invoice-embedded-errors');
            const saveButton = editorForm?.querySelector('button[type="submit"]:not([formaction])');

            document.getElementById('invoice-embedded-cancel')?.addEventListener('click', () => {
                window.parent.postMessage({ type: 'invoice-edit-cancel' }, window.location.origin);
            });

            editorForm?.addEventListener('submit', async (event) => {
                if (event.submitter?.formaction?.includes('preview')) {
                    return;
                }
                ...
                try {
                    ...
                    if (!response.ok) {
                        const errors = payload.errors
                            ...
                            : [payload.message || 'ذخیره فاکتور انجام نشد.'];
                        throw new Error(errors.join(' '));
                    }
                    ...
                    const message = error.message || 'ذخیره فاکتور انجام نشد.';

                    window.parent.postMessage({
                        type: 'invoice-validation-error',
                        message,
                    }, window.location.origin);

                    if (typeof window.showErpToast === 'function') {
                        window.showErpToast(message, 'danger');
                    } else if (embeddedErrors) {
                        embeddedErrors.hidden = false;
                        embeddedErrors.textContent = message;
                    }
                } finally {
                    if (saveButton) {
